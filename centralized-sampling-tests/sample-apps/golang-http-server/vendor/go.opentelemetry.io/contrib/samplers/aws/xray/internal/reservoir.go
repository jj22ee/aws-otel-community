...
func (r *reservoir) take(now time.Time, borrowed bool, itemCost float64) bool { // nolint: revive  // borrowed is not a control flag.
	...
	// log.Println(r.quotaBalance, " r.quotaBalance")
	// log.Println(r.quota, " r.quota")
	// log.Println(r.capacity, " r.capacity")
	// log.Println(r.lastTick.IsZero(), " r.lastTick.IsZero()")
	...
	if r.quotaBalance >= itemCost {
		...
		// log.Println(r.quotaBalance >= itemCost)
		return true
	}
...
func (r *reservoir) refreshQuotaBalanceLocked(now time.Time, borrowed bool) { // nolint: revive  // borrowed is not a control flag.
	...
	if borrowed {
		...
		// log.Println("r.quotaBalance", r.quotaBalance, "elapsedSeconds", elapsedSeconds)
