...
func (m *Manifest) RefreshManifestRules(ctx context.Context) (err error) {
	// log.Println("RefreshManifestRules")
...
func (m *Manifest) RefreshManifestTargets(ctx context.Context) (refresh bool, err error) {
	// log.Println("Updating RefreshManifestTargets")
	...
	if len(statistics) == 0 {
		// log.Println("no statistics to report and not refreshing sampling targets")
	...
	// log.Println("Updating updateTargets YAAAAaaaaaaaaaaaaaaaaaaaaay")
...
func (m *Manifest) updateRules(rules *getSamplingRulesOutput) {
	...
	for _, records := range rules.SamplingRuleRecords {
		...
		// log.Println("updateRules " + records.SamplingRule.RuleName)
		// log.Println(records.SamplingRule.ReservoirSize)
		// log.Println(records.SamplingRule.FixedRate)
...
func (m *Manifest) updateReservoir(t *samplingTargetDocument) (err error) {
	...
	// log.Println("Updating reservoir")
	// log.Println(len(m.Rules))
	...
	for index := range m.Rules {
		if m.Rules[index].ruleProperties.RuleName == *t.RuleName {
			...
			// log.Println(m.Rules[index].ruleProperties.RuleName + " == " + *t.RuleName)
			...
			if t.ReservoirQuota != nil {
				...
				// log.Println("quota - ", m.Rules[index].reservoir.quota)
			}
			if t.ReservoirQuotaTTL != nil {
				m.Rules[index].reservoir.expiresAt = time.Unix(int64(*t.ReservoirQuotaTTL), 0)
				// log.Println("expiresAt", m.Rules[index].reservoir.expiresAt)
			}
			if t.Interval != nil {
...
func (m *Manifest) snapshots() ([]*samplingStatisticsDocument, error) {
	...
	for index := range m.Rules {
		if m.Rules[index].stale(m.clock.now()) {
			...
			// log.Println(*s.RequestCount, " ", *s.SampledCount, " ", *s.BorrowCount, " ", *s.RuleName, " ", *s.Timestamp)
