...
public class CentralizedSamplingIntegrationTests {
    ...
    public static void makeDummyCalls(testCase testCase, SampleRule sampleRule) throws IOException {
        RequestBody reqbody = null;
        String stringResp = "";
        if (testCase.getMethod().equals("POST")) {
            reqbody = RequestBody.create(null, new byte[0]);
        ...
        logger.info(
                "Making Dummy Call for Sample Rule "
                        + sampleRule.getName().getSampleName()
                        + " and test case "
                        + testCase.getName());
        try (Response response =
                     httpClient()
                             .newCall(
                                     new Request.Builder()
                                             ...
                                             .addHeader(GenericConstants.REQUIRED, testCase.getRequired())
                                             .addHeader(
                                                     GenericConstants.TOTAL_SPANS, String.valueOf("10"))
                                             .url(host + testCase.getEndpoint())
                                             .method(testCase.getMethod(), reqbody)
                                             .build())
                             .execute()) {
